...
var (
	...
	ErrInvalidPaymentMethod = NewError(http.StatusBadRequest, "INVALID_PAYMENT_METHOD", "Choose a valid payment method: UPI, CARD, NET_BANKING, or QR_CODE.")
	ErrPaymentVerificationFailed = NewError(http.StatusBadRequest, "PAYMENT_VERIFICATION_FAILED", "Payment could not be verified. If money was debited, contact support with your payment reference.")
	ErrRazorpayProofRequired     = NewError(http.StatusBadRequest, "RAZORPAY_PROOF_REQUIRED", "Complete payment in the Razorpay window, then try again.")
